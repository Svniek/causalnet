...
export default function DataTab({ nodes, edges, influence, analysed, problem }) {
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#334155", marginTop: 2 }}>{factorRows.length} factoren &middot; hoofddoel: {center?.label}</div>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
              {["Factor", "Type", "Invloed %", "r Hoofddoel", ...subgoals.map(g => g.label.length > 16 ? g.label.slice(0, 14) + "\u2026" : g.label)].map((h, i) => (
